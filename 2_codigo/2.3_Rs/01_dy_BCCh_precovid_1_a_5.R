...
# =============================================================================
# Script: 01_dy_BCCh_precovid_1_a_5.R
...
ruta_output <- ruta / "3.2_resultado_dy/salidas_precovid"
...
dy_wide <- dy_long %>%
  ...
  arrange(periodo) %>%
  filter(periodo != "2003T1" & periodo < "2019T4") #pre covid.
...
p_tci <- ggplot(df_tci_rw, aes(x = fecha, y = tci, color = ventana, linetype = ventana)) +
  ...
  )  +  scale_color_manual(values = c("W=32" = "steelblue", "W=34" = "darkorange", "W=36" = "forestgreen")) +
...
p_pairwise <- ggplot(df_pairwise_rw, aes(x = fecha, y = pairwise_value, color = ventana, linetype = ventana)) +
  ...
  labs(
    ...
    subtitle = "Permite ver la direccion exacta del contagio entre estos dos actores",
...
cat("\nPROCESO COMPLETADO EXITOSAMENTE. GRAFICOS 1 A 9 GENERADOS.\n")
